...
bool MsManager::checkEffects(){
    ...
    for(std::vector<supervisor_msgs::MentalState>::iterator itms = msList_.begin(); itms != msList_.end(); itms++){
        ...
        for(std::vector<supervisor_msgs::Action>::iterator it = itms->todoActions.begin(); it != itms->todoActions.end(); it++){
            //we remove from effects the one with NULL
            std::vector<toaster_msgs::Fact> newEffects;
            for(std::vector<toaster_msgs::Fact>::iterator itf = it->effects.begin(); itf != it->effects.end(); itf++){
                if(itf->subjectId != "NULL" && itf->targetId != "NULL"){
                    newEffects.push_back(*itf);
                }
            }
            if(areFactsInTable(newEffects, itms->agentName, true)){
